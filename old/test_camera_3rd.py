    ...
    try:
        ...
        try:
            ...
            if not mode_switched:
                # 스와이프로 모드 전환 시도 (오른쪽에서 왼쪽으로)
                # 구글 카메라는 사진 -> 비디오로 갈 때 오른쪽에서 왼쪽으로 스와이프
                size = driver.get_window_size()
                start_x = int(size['width'] * 0.8)  # 오른쪽에서 시작
                end_x = int(size['width'] * 0.2)    # 왼쪽으로 이동
                y = int(size['height'] * 0.5)
                driver.swipe(start_x, y, end_x, y, 500)
                print("  ✓ 스와이프로 비디오 모드 전환 시도 (오른쪽→왼쪽)")
                time.sleep(2)
        except Exception as e:
